...
#include <unistd.h>
#include <sqlite3.h>
/* ESP-IDF v5: esp_spi_flash.h is gone.
 * - esp_random() lives in esp_random.h (pulled in by esp_system.h).
 * - SPI_FLASH_SEC_SIZE was in esp_spi_flash.h / spi_flash_mmap.h but the
 *   header's include path is fragile across IDF versions and PlatformIO
 *   package layouts.  The value is a hardware constant (4096 bytes on every
 *   ESP32 variant) so we define it directly to avoid the dependency.        */
#include <esp_system.h>
#include <esp_random.h>          /* esp_random() */
#include "esp_rom_sys.h"

#ifndef SPI_FLASH_SEC_SIZE
#define SPI_FLASH_SEC_SIZE  4096 /* flash erase-sector size, bytes */
#endif
#include <sys/stat.h>
...
int esp32_Open( sqlite3_vfs * vfs, const char * path, sqlite3_file * file, int flags, int * outflags )
{
    ...
    /* FIX: fopen returns NULL on failure, not 0 / negative */
    if ( p->fd == NULL ) {
		return SQLITE_CANTOPEN;
	}
...
int esp32_Randomness( sqlite3_vfs * vfs, int len, char * buffer )
{
	...
	while (sz--) {
        rdm = (long)esp_random();   /* FIX: esp_random() returns uint32_t; cast to long */
...
{
	/* FIX: ets_delay_us() is gone from public ROM headers in IDF v5.
	 * Use esp_rom_delay_us() which is always available.              */
